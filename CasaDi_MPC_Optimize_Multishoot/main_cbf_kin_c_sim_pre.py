...
import MPC_CBF_optimize_kin_pre
...
if __name__ == "__main__":
    ...
    # Simulation
    # initialize
    mpc_solver = MPC_CBF_optimize_kin_pre.MPC_optimize()
    ...
    obs = [np.array([[50, 3.5, 0, 10, 4.8, 1.8]])] # 障碍物位置速度+长宽
    obs_trajectories = obs_prediction(obs, T_S, N_p)
    xsq = []
    usq = []
    usq0 = []
    refsq = []
    ...
    if solve_flag:
        while( mpciter-sim_time/T_S < 0.0):
            ...
            obs_trajectories_dyn = obs_prediction(obs, T_S, N_p)
            solver = mpc_solver.optimize_problem(ego_state=x0, ref_state=ref_traj, obs_trajectories=obs_trajectories_dyn)
            ...
            # update obs
            obs = [obs_trajectories_dyn[0][1].reshape(1, -1)]
            if mpciter == 0:
            # if x_m[0, 0] > 0:
                xsq.append(x_m.T)
                usq.append(u0.T)
                ...
                obs_trajectories = obs_trajectories_dyn
    ...
    if plot_opt_sq_flag:
        ...
        for i, trajectory in enumerate(obs_trajectories):
            ...
            axxy.plot(x_values, y_values, 'g--')
            for j in range(0, len(x_values), 10):
                x_center = x_values[j]
                y_center = y_values[j]
                ellipse = Ellipse(
                    (x_center, y_center), width=trajectory[j, 4], height=trajectory[j, 5],
                    angle=np.degrees(trajectory[j, 2]), color='orange', fill=False, linewidth=2
                )
                axxy.add_patch(ellipse)
